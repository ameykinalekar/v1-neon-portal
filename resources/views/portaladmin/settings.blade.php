    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-body py-1">
                <h4 class="page-title"> <i class="mdi mdi-settings title_icon"></i>System Settings</h4>
            </div> <!-- end card body-->
        </div> <!-- end card -->
    </div><!-- end col-->
</div>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <form method="POST" class="col-12 systemLogoAjaxForm" action="" id="system_settings"
                    ...
                    @csrf
                    <input type="hidden" name="setting_id" value="{{ $response['result']['settings']['setting_id']??'' }}">
                    <div class="row justify-content-center">
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                                <label class="form-label" for="system_title">System Title</label>
                                <input type="text" id="system_title" name="system_title" class="form-control" value="{{ $response['result']['settings']['system_title']??'' }}">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                                <label class="form-label" for="system_email">System Email</label>
                                <input type="email" id="system_email" name="system_email" class="form-control" value="{{ $response['result']['settings']['system_email']??'' }}">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                                <label class="form-label" for="phone">Phone</label>
                                <input type="tel" id="phone" name="phone" class="form-control" onkeypress="return isPhone(event);" value="{{ $response['result']['settings']['phone']??'' }}"   title="Please use a 10 digit telephone number with no dashes or dots" pattern="\+?[0-9]{10,12}" ><small><i>10 digit telephone number with no dashes or dots.</i></small>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                                <label class="form-label" for="address">Address</label>
                                <input type="text" id="address" name="address" class="form-control" value="{{ $response['result']['settings']['address']??'' }}">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                                <label class="form-label" for="footer_text">Footer Text</label>
                                <input type="text" id="footer_text" name="footer_text" class="form-control" value="{{ $response['result']['settings']['footer_text']??'' }}">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                                <label class="form-label" for="footer_link">Footer Link</label>
                                <input type="text" id="footer_link" name="footer_link" class="form-control" value="{{ $response['result']['settings']['footer_link']??'' }}">
                            </div>
                        </div>
                        @php
                        ...
                        <div class="col-md-4">
                            <div class="form-group  mb-3">
                                <label class="form-label" for="main_logo">Regular Logo</label>
                                <div class="js--image-preview"
                                    style="background-image: url(<?php echo config('app.api_asset_url') . $response['result']['settings']['main_logo'] ?? ''; ?>); background-color: #F5F5F5;">
                                </div>
                                <input type="file" class="form-control" name="main_logo" id="main_logo"
                                    accept="image/x-png,image/jpeg,image/png;capture=camera">
                                <small id="imgSizeMainLogo">Please upload file size 300 X 300 (Pixels)</small>
                                <img id="main_logo_show_image" width='100%' src="">
                                <input id="imagedata_main_logo" type="hidden" class="form-control input-border-bottom"
                                    name="imagedata_main_logo" value="">
                        ...
                        <div class="col-md-4">
                            <div class="form-group  mb-3">
                                ...
                                <div class="js--image-preview"
                                    style="background-image: url(<?php echo config('app.api_asset_url') . $response['result']['settings']['favicon'] ?? ''; ?>); background-color: #F5F5F5;">
                                </div>
                                <input type="file" class="form-control" name="favicon" id="favicon"
                                    accept="image/x-png,image/jpeg,image/png;capture=camera">
                                <small id="imgSizeFavicon">Please upload file size 50 X 50 (Pixels)</small>
                    ...
                    <div class="text-center">
                        <button type="submit" class="btn btn-secondary col-xl-4 col-lg-6 col-md-12 col-sm-12" onclick="cropme();" >Update Settings</button>
                    </div>
                </form>
            </div> <!-- end card body-->
        </div>
    ...
    $("#main_logo").change(function() {
        if (this.files && this.files[0]) {
            if (this.files[0].type.match('image.*')) {
                ...
                cropParamLogo['imageId'] = "main_logo_show_image";
                cropParamLogo['dataImageId'] = "imagedata_main_logo";
...
function cropme() {
    var srcOriginal;
    if (document.getElementById('main_logo').files.length >0){
        srcOriginal = $('#main_logo_show_image').rcrop('getDataURL',300,300);
        $('#imagedata_main_logo').val(srcOriginal);
