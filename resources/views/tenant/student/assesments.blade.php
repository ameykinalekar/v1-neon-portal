        ...
        <div class="row">
            ...
            <div class="col-md-6 col-lg-2 my-2 px-2 grid">
                <div class="card" style="width: 100%; border-radius: 4px !important;">
                    ...
                    <div class="card-body">
                        <h5 class="card-title">
                            {{$record['name']}}<br><small>{{$record['subject']['subject_name']  }} ({{$record['subject']['yeargroup']['name']}} : {{$record['subject']['academicyear']['academic_year']}})<p>{{$record['lesson_name']??''}}</p></small>
                        </h5>
                        <div class="lp-course-buttons">
                         @if($record['is_submitted'])
                         <a href="javascript:void(0);" class=" lp-button button button-purchase-course">
                                Attempted
                            </a>
                         @else
                         <a href="<?php echo config('app.base_url') . '/' . Session()->get('tenant_info')['subdomain'] . '/start-assessment'; ?>/{{ \Helpers::encryptId($record['examination_id']) }}" class="lp-button button button-purchase-course">
                                Open Test
                            </a>
                        @endif
                        </div>
        ...
        <table id="datatable" class="table table-striped  nowrap table list" width="100%">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{$record['subject']['subject_name']}}<small><p>{{$record['lesson_name']??''}}</p></small></td>
                    ...
                    @if($record['is_submitted'])
                         ...
                         @else
                            <a href="<?php echo config('app.base_url') . '/' . Session()->get('tenant_info')['subdomain'] . '/start-assesment'; ?>/{{ \Helpers::encryptId($record['examination_id']) }}" class="lp-button button button-purchase-course">
                                Open Test
                            </a>
                        @endif
                    </td>
