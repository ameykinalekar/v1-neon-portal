...
@section('title', 'AI Addaptive Learning Plan')
@section('pagecss')
<style type="text/css">
.rotate-me {
            ...
            <div class="card-body admin_content">
                <div class="table-responsive">
                    <table class="table table-bordered wrap" width="100%">
                        ...
                        @php $lastTopicId=''; @endphp
                        @foreach($listing as $record)
                        @if($lastTopicId!=$record['topic_id'])
                        <tr>
                            <td class="rotate-me" style="text-align:center;font-weight:700;vertical-align: middle" rowspan="{{$record['sub_topics_count']+1}}">{{$record['topic']??''}}
                            </td>
                        </tr>
                        @endif

                        <tr>
                            <td>{{$record['sub_topic']}}</td>
                            <td class="text-center">
                                @if($record['tc']>80)
                                <span style="font-size:50px;color:green">&bull;</span>
                                @elseif($record['tc']>50 && $record['tc'] < 80)
                                <span style="font-size:50px;color:orange;">&bull;</span>
                                @elseif($record['tc']>0 && $record['tc'] < 50)
                                <span style="font-size:50px;color:red;">&bull;</span>
                                @else
                            ...
                            <td class="text-center">
                                @if($record['ms']>80)
                                <span style="font-size:50px;color:green">&bull;</span>
                                @elseif($record['ms']>50 && $record['ms'] < 80)
                                <span style="font-size:50px;color:orange;">&bull;</span>
                                @elseif($record['ms']>0 && $record['ms'] < 50)
                                <span style="font-size:50px;color:red;">&bull;</span>
                                @else
                            ...
                            <td class="text-center">
                                @if($record['ps']>80)
                                <span style="font-size:50px;color:green">&bull;</span>
                                @elseif($record['ps']>50 && $record['ps'] < 80)
                                <span style="font-size:50px;color:orange;">&bull;</span>
                                @elseif($record['ps']>0 && $record['ps'] < 50)
                                <span style="font-size:50px;color:red;">&bull;</span>
                                @else
                            ...
                            <td class="text-center">
                                @if($record['at']>80)
                                <span style="font-size:50px;color:green">&bull;</span>
                                @elseif($record['at']>50 && $record['at'] < 80)
                                <span style="font-size:50px;color:orange;">&bull;</span>
                                @elseif($record['at']>0 && $record['at'] < 50)
                                <span style="font-size:50px;color:red;">&bull;</span>
                                @else
                        ...
                        @php $lastTopicId=$record['topic_id']; @endphp
                        @endforeach
                    </table>
                </div>
...
$("#subject_id").on('change', function() {
    ...
    params['url'] =
        "<?php echo config('app.api_base_url') . '/' . Session()->get('tenant_info')['subdomain'] . '/teacher/students-by-subjectid'; ?>";
    params['requestType'] = "POST";
    params['dataType'] = "json";
    params['contentType'] = "application/json; charset=utf-8";
    params['headers'] = {
    ...
    params['errorCallBackFunction'] = function(httpObj) {
        $('#user_id').html('<option value="">Select your choice</option>');
    }
    params['completeCallbackFunction'] = function(response) {
        var existingValue = "{{$user_id ?? ''}}";
        if (existingValue != '') {
            $('#user_id').val(existingValue);
...
$('#lesson_id').on('change', function() {
    if($('#user_id').val()!=''){
        ...
        alert('Please select student for information.');
        $('#lesson_id').val('');
    }
});
</script>
