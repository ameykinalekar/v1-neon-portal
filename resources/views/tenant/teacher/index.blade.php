        ...
        <div class="card">
            <div class="card-body py-1">
                ...
                <button type="button" class="btn btn-outline-primary btn-rounded align-middle mt-1 float-end"
                    onclick="rightModal('{{route('ta_addteacher',Session()->get('tenant_info')['subdomain'])}}', 'Add Teacher')">
                    <i class="mdi mdi-plus"></i> Add Teacher</button> <a class="btn btn-outline-primary btn-rounded align-middle mt-1 float-end mx-2"
                    href="javascript:void(0);" onclick="rightModal('{{route('ta_importteacher',Session()->get('tenant_info')['subdomain'])}}', 'Import Teachers')"><i class="mdi mdi-upload"></i> Import</a>
            </div> <!-- end card body-->
        </div> <!-- end card -->
    </div><!-- end col-->
</div>
                        ...
                        <div class="input-group input-group-sm">
                            ...
                            <div class="input-group-btn">
                                ...
                                <a class="btn btn-light"
                                    href="{{route('ta_teacherlist',Session()->get('tenant_info')['subdomain'])}}">Reset</a>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <table id="datatable" class="table table-striped  nowrap" width="100%">
                    <thead>
                        <tr style="background-color:rgba(90, 194, 185, 1); color: #ffffff;">
                            ...
                            <th width="10%">Teacher Email</th>
                            <th width="16%">Year Group</th>
                            <th width="16%">Subject</th>
                            <th width="8%">DBS</th>
                            <th width="10%">Govt. ID</th>
                            <th width="9%">Status</th>
                            <th>Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        @if(count($response['result']['teacher_list']['data'])>0)
                        @foreach($response['result']['teacher_list']['data'] as $record)
                        @php
                            $yrgrp=array();
                            $subjects=array();
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="javascript:void(0);" title="Edit Teacher"
                                    onclick="rightModal('{{route('ta_editteacher',[Session()->get('tenant_info')['subdomain'],\Helpers::encryptId($record['user_id'])])}}', 'Edit Teacher')"><i
                        ...
                        <nav class="float-end">
                            <ul class="pagination">
                                ...
                                <li class="page-item"><a class="page-link" title="Next Page"
                                        href="{{route('ta_teacherlist',Session()->get('tenant_info')['subdomain']).'/'.$initialQS.'&page='.$next_page}}"><span
                                            aria-hidden="true">&raquo;</span></a></li>
                                @elseif(($has_next_page == false) && ($has_previous_page == true))
                                <li class="page-item"><a class="page-link" title="Previous Page"
                                        href="{{route('ta_teacherlist',Session()->get('tenant_info')['subdomain']).'/'.$initialQS.'&page='.$prev_page}}"><span
