...
<form method="POST" class="d-block ajaxForm" id="frmAddQuestion"
    ...
    <input type="hidden" name="examination_question_id" value="{{$examination_question_id}}">
    <input type="hidden" name="question_id"
        value="{{$examination_question_details['examination_question']['question_id']}}">
    <?php //dd($examination_question);?>
    <div class="row">
        ...
        <div class="col-md-4 mb-2">
            <div class="form-group required">
                ...
                <input type="number" step="any" name="point" id="point" class="form-control" required
                    value="{{$examination_question_details['examination_question']['point']}}">
            </div>
        ...
        <div class="col-md-4 mb-2">
            <div class="form-group required">
                ...
                <input type="number" step="any" name="time_inseconds" id="time_inseconds" class="form-control" required
                    value="{{round(($examination_question_details['examination_question']['time_inseconds']??0)/60,2)}}">
            </div>
        </div>
        <div class="col-md-8 mb-2">
            <div class="form-group required">
                ...
                <div class="row">
                    @if($examination_question_details['examination_question']['tc']??''==1)
                    <div class="col-md-3 mb-2"><input type="checkbox" name="tc" checked value="1" class="float-start"><label
                    ...
                    <div class="col-md-3 mb-2"><input type="checkbox" name="tc" value="1" class="float-start"><label
                            class="float-start px-1" style=";font-weight:500;">TC</label></div>
                    @endif
                    @if($examination_question_details['examination_question']['ms']??''==1)
                    <div class="col-md-3 mb-2"><input type="checkbox" name="ms" checked value="1" class="float-start"><label
                    ...
                    <div class="col-md-3 mb-2"><input type="checkbox" name="ms" value="1" class="float-start"><label
                            class="float-start px-1" style="font-weight:500;">MS</label></div>
                    @endif
                    @if($examination_question_details['examination_question']['ps']??''==1)
                    <div class="col-md-3 mb-2"><input type="checkbox" name="ps" checked value="1" class="float-start"><label
                    ...
                    <div class="col-md-3 mb-2"><input type="checkbox" name="ps" value="1" class="float-start"><label
                            class="float-start px-1" style="font-weight:500;">PS</label></div>
                    @endif
                    @if($examination_question_details['examination_question']['at']??''==1)
                    <div class="col-md-3 mb-2"><input type="checkbox" name="at" checked value="1" class="float-start"><label
        ...
        <div class="col-md-12 mb-2">
            <div class="form-group required">
                ...
                <textarea name="question" id="question" cols="30" rows="3" class="form-control ckeditor"
                    required>{{$examination_question->question}}</textarea>
            </div>
        </div>
        ...
        <div id="answer_fields" class="optsec">
            @if(isset($examination_question->options) && is_array($examination_question->options))
            @for($i=0; $i<count($examination_question->options); $i++)
                <div class="row mb-2">
                    <div class="col-md-7">
                        <input type="text" class="form-control" name="qoptions[]"
                            value="{{$examination_question->options[$i]->option_value}}">
                        <input type="hidden" class="form-control" name="qcorrect[]"
                            value="{{$examination_question->options[$i]->is_correct}}" id="qcorrect$i">
                    </div>
                    <div class="col-md-3">
                        @if($examination_question->options[$i]->is_correct>0)
                        <input type="radio" id="anschk$i" name="is_correct[]" data-id="$i" class="form-check"
                            onclick="check(this);" checked> <label>Correct</label>
                        @else
                        <input type="radio" id="anschk$i" name="is_correct[]" data-id="$i" class="form-check"
...
$(document).ready(function() {
    ...
    var existingValue = "{{$examination_question->require_file_upload ?? ''}}";
    $('#require_file_upload').val(existingValue).trigger('change');
...
function onPageLoad() {
    ...
    params['completeCallbackFunction'] = function(response) {
        var existingValue = "{{$examination_question->question_type ?? ''}}";
        if (existingValue != '') {
            $('#question_type').val(existingValue).trigger('change');
        }
    }
    ...
    params['completeCallbackFunction'] = function(response) {
        var existingValue = "{{$examination_question->level ?? ''}}";
        if (existingValue != '') {
            $('#level').val(existingValue).trigger('change');
        }
    }
...
$("#lesson_id").on('change', function() {
    ...
    params['completeCallbackFunction'] = function(response) {
        var existingValue = "{{$examination_question_details['examination_question']['topic_id'] ?? ''}}";
        if (existingValue != '') {
            $('#topic_id').val(existingValue).trigger('change');
...
$("#topic_id").on('change', function() {
    ...
    params['completeCallbackFunction'] = function(response) {
        var existingValue = "{{$examination_question_details['examination_question']['sub_topic_id'] ?? ''}}";
        if (existingValue != '') {
            $('#sub_topic_id').val(existingValue).trigger('change');
