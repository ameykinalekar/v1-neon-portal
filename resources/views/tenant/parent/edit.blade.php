...
<form method="POST" class="d-block ajaxForm"
    action="{{route('ta_updateparent',Session()->get('tenant_info')['subdomain'])}}">
    @csrf
    <input type="hidden" name="parent_id" id="parent_id" value="{{$details['user_id']??''}}">
    <div class="form-row">
        ...
        <div class="form-group mb-1">
            ...
            <input type="text" class="form-control" id="phone" name="phone" value="{{$details['phone']??''}}"
        ...
        <div class="form-group mb-1">
            ...
            <select class="form-control" id="gender" name="gender" required>
                ...
                @if(isset($details['gender']) && $details['gender']==$record)
        ...
        <div class="form-group mb-1">
            ...
            <textarea class="form-control" id="address" name="address"
                rows="5">{{$details['address']??''}}</textarea>
        </div>
        ...
        <div class="form-group mb-1">
            ...
            <div class="custom-file-upload">
                <input type="file" class="form-control" id="profile_image" name="profile_image"
                    accept="image/x-png,image/jpeg,image/png;capture=camera">                <h6>Please upload file size 200 x 200 (Pixels)</h6>


                @if($details['user_logo']!='')
                <span>
                    <a class="fancy-box-a" data-fancybox="demo" data-caption="Profile Image"
                        href="{{config('app.api_asset_url') . $details['user_logo']}}"><img
                            style="padding-top: 13px;"
                            src="{{config('app.api_asset_url') . $details['user_logo']}}" height="auto"
                            width="70px" /></a>
                </span>
                @endif
